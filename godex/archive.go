...
func BuildArchiveBatchLabel(input ArchiveBatchLabel, options LabelOptions) []string {
	...
	qtyText := SanitizeLabelText(input.QtyText)
	batchTime := SanitizeLabelText(input.BatchTime)
	if sessionID == "" {
		sessionID = "-"
	}
	if itemName == "" {
