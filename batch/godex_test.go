...
func TestBuildArchiveBatchLabelContainsQRAndText(t *testing.T) {
	data, err := BuildArchiveBatchLabel(ArchiveBatchLabel{
		SessionID: "sess-1",
	...
	if err != nil {
		t.Fatalf("BuildArchiveBatchLabel: %v", err)
	}
	joined := strings.Join(data.Commands, "\n")
	for _, want := range []string{
		"^Q50,3",
	...
	for _, notWant := range []string{"BA,", "COMPANY:", "EPC:", "BATCH INFO"} {
	...
	if !strings.HasPrefix(data.QRPayload, DefaultArchiveQRBaseURL) {
		t.Fatalf("archive qr payload = %q", data.QRPayload)
	}
}
...
func TestPrinterPrintArchiveBatchSendsGraphicsBeforeCommands(t *testing.T) {
	ft := &fakeTransport{}
	p := NewPrinter(ft)
	_, err := p.PrintArchiveBatch(ArchiveBatchLabel{
		SessionID: "sess-1",
		ItemName:  "Zo'r chips 5D shashlik",
		QtyText:   "4.2",
		BatchTime: "01 May 2026 15:23",
	...
	if firstY := strings.Index(joined, "Y0,0,TEXTLBL"); firstY < strings.Index(joined, "~EB,QRLBL,") {
		t.Fatalf("archive label command sent before qr graphic download:\n%s", joined)
	}
	if len(ft.rawWrites) != 2 {
